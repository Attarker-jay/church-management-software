...
$database = new Database;
//Query
$database->query('SELECT * FROM welfarecontribution');
...
<body style="background-image: url('../img/bg-01.jpg'); background-repeat: no-repeat; background-size: cover;">
    <div class="page-wrapper default-theme sidebar-bg bg1 toggled">
        ...
        <main class="page-content">
          ...
          <h3 style="font-variant: small-caps;font-size: 40px; padding-left: 150px"><strong>Analyzing Welfare Contributions</strong></h3><hr>
          <p></p>
          ...
          if($SearchRows){
            foreach( $SearchRows as $SearchRowsGet){
             ...
             <tbody>
             <tr class='table-info'><td>{$SearchRowsGet['id']}</td><td>{$SearchRowsGet['first_name']}</td><td>{$SearchRowsGet['last_name']}</td><td>{$SearchRowsGet['email']}</td><td>{$SearchRowsGet['location']}</td><td>{$SearchRowsGet['tel']}</td><td>{$SearchRowsGet['amount']}</td><td>{$SearchRowsGet['registered_date']}</td></tr>\n";
         }
     }elseif(!empty($search)){
...
<tbody>
   ...
   if($rows){
    foreach( $rows as $row){
      echo "<tr><td>{$row['id']}</td><td>{$row['first_name']}</td><td>{$row['last_name']}</td><td>{$row['email']}</td><td>{$row['location']}</td><td>{$row['tel']}</td><td>{$row['amount']}</td><td>{$row['registered_date']}</td></tr>\n";
  }
}else{
 echo '<div class="alert alert-danger" role="alert">
 <p class="error">No Welfare Contributions Available *Use offering section to add one* !</p>
 </div>';
}
?>
</tbody>
